/* ═══════════════════════════════════════════════════════════════════════════
   P11 — Exercice n°1 (Capteurs, actionneurs & CDCF) — report des notes validées
   À coller dans le classeur « Evaluation P11 — Exercice n°1 (Capteurs, actionneurs & CDCF) » :
   ...
   classe, élève (« Prénom NOM » du compte du site), points des 8 parties (sur 30), note et barème.
...
var SECRET = "P11-EX1-2026";             // le même que dans index.html du site
var CLASSES = ["4A", "4B", "4C", "4D", "4E", "4F", "4G"];
var PREMIERE_LIGNE = 4;                  // lignes 1 à 3 : en-têtes et barème
var PARTIES = ["cap", "intf", "act", "ci", "ce", "bete", "pie", "cdcf"];            // colonnes B à I
var TITRES = ["A1 Capteurs", "A2 Interfaces", "A3 Actionneurs", "A4 Chaîne d'info", "A5 Chaîne d'énergie",
              "B Bête à cornes", "C Pieuvre", "D CDCF"];
var MAX = [3, 3, 3, 3, 4, 5, 5, 4];
var COL = { nom: 1, pts: 2, note: 12, date: 13, appr: 14, remarque: 15 };           // A, B-I, L, M, N, O
...
function doPost(e) {
  ...
  try {
    ...
    if (ligne < 0) {
      ...
      var saisie = feuille.getRange(ligne, COL.pts, 1, 8).getValues()[0].some(function (v) { return v !== ""; });
      var dejaSite = String(feuille.getRange(ligne, COL.remarque).getValue()).indexOf(MARQUE) === 0;
      statut = (saisie && !dejaSite) ? "manuel" : "note";
    }
    if (statut !== "manuel") {
      feuille.getRange(ligne, COL.pts, 1, 8).setValues([pts]);
...
function doGet() {
  return ContentService.createTextOutput("P11 Exercice n°1 : script de report en ligne.");
}
...
function installer() {
  ...
  mode.getRange(1, 1, 9, 1).setValues([
    ["P11 — Exercice n°1 (Capteurs, actionneurs & CDCF) — Report des notes (4e)"], [""],
    ["Un onglet par classe. Colonne A : coller la liste des élèves (NOM Prénom, export Ecole Directe) à partir de la ligne 4."],
    ["B à I : points de chaque partie, remplis automatiquement quand le professeur valide la copie dans le modal du site Technologie."],
    ...
    ["J : total sur 30 ; K : note sur 20 (demi-point le plus proche, comme le site) ; L : note validée sur le site, avec le barème choisi."],
    ["Couleurs : bleu > 88 % · vert > 64 % · jaune > 30 % · rouge ≤ 30 %."],
    ["Aide : techno-p11-eau/apps-script/LISEZ-MOI-EX1.md"]]);
  ...
  CLASSES.forEach(function (classe) {
    ...
    var entetes = [["Nom et prénom"].concat(TITRES, ["TOTAL", "NOTE", "Note validée", "Validée le", "Appréciation", "Remarque"]),
                   ...
                   [""].concat(MAX, [30, 20, "", "", "", ""])];
    f.getRange(1, 1, 3, 15).setValues(entetes).setFontWeight("bold").setHorizontalAlignment("center")
      .setVerticalAlignment("middle").setWrap(true).setBackground("#D8D8D8");
    f.getRange(1, 10, 3, 2).setBackground("#A4C2F4");
    f.getRange("A1:A3").merge().setFontSize(13);
    var formules = [];
    for (var i = 4; i < 4 + LIGNES; i++) {
      formules.push(['=IF(COUNT(B' + i + ':I' + i + ')=0;"";SUM(B' + i + ':I' + i + '))',
                     '=IF(J' + i + '="";"";ROUND(J' + i + '*20/30*2;0)/2)']);
    }
    if (!/^fr/.test(ss.getSpreadsheetLocale())) formules = formules.map(function (l) {
      return l.map(function (x) { return x.replace(/;/g, ","); });
    });
    f.getRange(4, 10, LIGNES, 2).setFormulas(formules).setFontWeight("bold");
    f.getRange(4, 2, LIGNES, 11).setHorizontalAlignment("center");
    f.getRange(1, 1, 3 + LIGNES, 15).setBorder(true, true, true, true, true, true, "#BFBFBF", SpreadsheetApp.BorderStyle.SOLID);
    var regles = [];
    ["J", "K"].forEach(function (col) {
    ...
    f.setColumnWidth(1, 230); for (var c = 2; c <= 12; c++) f.setColumnWidth(c, 78);
    f.setColumnWidth(13, 130); f.setColumnWidth(14, 380); f.setColumnWidth(15, 260);
  });
  var j = ss.getSheetByName("Journal") || ss.insertSheet("Journal");
  if (j.getLastRow() === 0) {
    j.appendRow(["Horodatage", "Classe", "Élève", "Total /30", "Note validée", "Points A1 à D", "Statut"]);
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu("P11 Exercice 1")
...
function testerEnvoi() {
  var r = doPost({ postData: { contents: JSON.stringify({ secret: SECRET, classe: "4A", eleve: "Essai TEST",
    pts: { cap: 3, intf: 2, act: 3, ci: 2.5, ce: 3, bete: 4, pie: 4.5, cdcf: 3 }, note: "16,5", bareme: 20,
